...
describe 'Kookaburra Integration' do
  describe "testing a Rack application" do
    describe "with an HTML interface" do
      describe "with a JSON API" do
        let(:my_api_driver) {
          Class.new(Kookaburra::JsonApiDriver) do
        ...
        let(:my_given_driver) {
          Class.new(Kookaburra::GivenDriver) do
            def a_user(name)
        ...
        let(:my_ui_driver) {
          # Reassignments necessary to give access to the variables in the class
          # definition block below. This is kind of lame.
          sign_in_screen = sign_in_screen_class
          widget_list = widget_list_class
          widget_form = widget_form_class
          Class.new(Kookaburra::UIDriver) do
            ui_component :sign_in_screen, sign_in_screen
            ui_component :widget_list, widget_list
            ui_component :widget_form, widget_form
  ...
  describe "testing a Ruby API" do
    let(:my_ui_driver) {
      Class.new(Kookaburra::UIDriver)
    }

    let(:my_given_driver) {
      Class.new(Kookaburra::GivenDriver)
    }

    let(:my_api_driver) {
      Class.new(Kookaburra::APIDriver)
    }
